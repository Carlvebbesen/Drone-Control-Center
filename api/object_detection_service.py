...
def find_detensions(inspectionId, db, firebase):
    ...
    while True:
        ...
        if(len(results)> 0 and len(results[0].boxes) > 0):
            print("Found single detension")
            if(int((frameCounter-lastResultsFrame)/fps) > 4):
                lastResultsFrame = frameCounter
                detensionObj = {
                    "inspectionId": inspectionId,
                    "detensionCount": len(detensionState),
                    "isValid": True,
                }
                print("trying to upload detension .... ")
                update_time, doc_ref = db.collection("detension").add(detensionObj)
                for finding in detensionState:
                    print(f"adding finding: {finding}")
                    db.collection("detension").document(doc_ref.id).collection("findings").add(finding)
                detensionState = []
            singleDetension= []
            for detensions in results[0].boxes:
                    singleDetension.append({
                        "name":fr"{results[0].names[int(detensions.cls[0])]}",
                        "conf": detensions.conf.item(),
                        })
            detension_hash = id(singleDetension)
            PATH = f"inspections/{inspectionId}"
            if not os.path.exists(PATH):
                os.makedirs(PATH)
            img_url =r"{0}/{1}{2}.jpg".format(PATH, detension_hash, frameCounter)
            detensionState.append({
                "detensions": singleDetension,
                "frame": frameCounter,
                "imgId": img_url,
            })
            print("Saving image")
